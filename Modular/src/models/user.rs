...
use rocket_db_pools::sqlx::{FromRow, PgConnection, Acquire};
use uuid::Uuid;
use std::error::Error;
...
pub struct NewUser<'r> {
    ...
    pub username: &'r str,
    #[field(validate = validated_email().or_else(msg ! ("invalid email")))]
    pub email: &'r str,
    #[field(validate = validated_password().or_else(msg!("week password")))]
    pub password: &'r str,
...
impl User {
    ...
    {
        return if pagination.is_some() {
            Self::find_all_with_pagination(db, &(pagination.unwrap())).await
        } else {
            Self::find_all_without_pagination(db).await
        };
    }
    ...
    async fn find_all_with_pagination(
        db: &mut Connection<DBConnection>,
        pagination: &Pagination) -> Result<(Vec<Self>, Option<Pagination>), Box<dyn Error>> {
    ...
    pub async fn create<'r>(
        ...
        let password_hash = argon2.hash_password(new_user.password_confirmation.as_bytes(), &salt);
        if password_hash.is_err() {
            return Err("Connection create password hash".into());
        }
        let query_str = r#"
...
fn validated_password(password: &str) -> form::Result<'_, ()> {
...
fn validated_email(email: &str) -> form::Result<'_, ()> {
